// src/components/home/ProductRange.tsx
  ...
  const products = [
    {
      ...
      href: '/polycarbonate-window-canopy-door-awning',
    ...
    },
    {
      ...
      href: '/suspended-polycarbonate-canopy',
      // 🌟 完美修复：把原本调皮的 </circle> 修正为正确的 </rect> 闭合标签！
    ...
    },
    {
      ...
      href: '/inside-corner-canopy-polycarbonate',
    ...
    },
    {
      ...
      href: '/corner-extended-polycarbonate-canopy',
    ...
    <section className="py-20 px-4 bg-gradient-to-tr from-[#d4f0e9] via-[#f2faf8] to-white">
      <div className="container mx-auto max-w-[1200px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {products.map((item) => {
            // 🌟 核心修复：干净漂亮地解开嵌套的 JSON 节点，彻底规避 TypeScript 跨级检索报错
            ...
            return (
              <article key={item.id} className="flex flex-col group">
                ...
                {/* 🌟 这里的 Link 平替为普通的 <a> 标签，路径无缝拼接当前语言 */}
                <a href={`/${lang}${item.href}`} className="w-full rounded-lg overflow-hidden mb-6 block relative">
                  <img 
                    src={item.img} 
                    alt={title} 
                    title={title}
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="text-[22px] font-bold text-[#184e77] mb-2">
                      <a href={`/${lang}${item.href}`} className="hover:underline">{title}</a>
                    </h3>
